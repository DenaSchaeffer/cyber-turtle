...
print_usage(){
    ...
    echo "Example : $0 -d src -e js -l nodejs -o sourcecode.md"
} 
...
print_result(){
    ...
    extension=$2
    language=$3    
    lookup_dir="`pwd`/$directory"    
    files=`find $lookup_dir  -name "*.$extension" -not -path ".*" -not -path "./*/node_modules*"`
    file_count=0
    for file in $files
    do 
...
write_result(){
    ...
    echo "# CPS 491 - Capstone II" > $output
    echo "This file is automatically created by a script. Please delete this line and replace with the course and your team information." >> $output
    for file in $files
    do 
        found_file=$(echo "$file" | sed "s|$lookup_dir||g")
